	...
	$.widget('ui.spinner', {
		...
		_parse: function(val) {
			...
			if (typeof val == 'string' && val != "") {
				...
				result = parseInt((val[0] || 0) == "-" ? "-0" : val[0] || 0, this.options.radix);
				if (val.length > 1) {
		...
		_format: function(num) {
			...
			if (isNaN(num)) {
				...
				var s = Number(Number(num).toFixed(maxdec)).toString();
		...
		//Modified by Randy 2010.4.12
		//Add new property "step"
		step: function(newVal) {
			if (!arguments.length) {
				return this.options.step;
			}
			this.options.step = newVal;
		},

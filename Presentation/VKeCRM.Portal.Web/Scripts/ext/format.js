...
vkecrm.RegisterNameSpace("Ext.util");
    ...
    return {
		...
		*/
        date: function (v, format) {
            ...
            var v2 = v;
            if (typeof (v2) == "string" && v2.indexOf('Date') > -1) {
                v = new Date(parseInt(v2.replace("/Date(", "").replace(")/", ""), 10));
            }
                ...
                if ($.browser.msie && $.browser.version == 8) {
                    if (v == "NaN") {
                        if (v2._d) {
                            ...
                            var times = v2.split('T');
                            var date = times[0];
                            var time = times[1].substring(0, 8).split(':');
                            var vv = date.split("-");
                            v2 = vv[1] + "-" + vv[2] + "-" + vv[0];
                            v2 = new Date(v2).addHours(time[0] * 1).addMinutes(time[1] * 1).addSeconds(time[2] * 1);
                            v = v2;
                        }
                    }
                }
        ...
        MSDateConvert: function (jsonDateString) {
